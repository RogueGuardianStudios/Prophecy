...
(no `SceneDescriptor`, so world-scene discovery and transitions ignore it). The whole widget
tree is **built at runtime in code** (`UiBuild` — the TransitionVeil precedent scaled up), so
there is no serialized hierarchy to drift and nothing for regeneration to clobber.

**Ported to UI Toolkit 2026-08-11 (Matt: "move everything to UI tool kit").** The scene holds
one `UIDocument` (no UXML — the tree is still code) plus the two controllers; the builder
also generates `Assets/_Prophecy/UI/PanelSettings.asset` (1920×1080 scale-with-screen, the
same design space as the old canvas, so every layout number kept its meaning) and
`UnityDefaultRuntimeTheme.tss` (the documented one-line import — what gives Labels a font
without shipping one). Each controller owns a named layer under the shared root
(`HudLayer` under, `MenuLayer` over), replaced wholesale on enable. The port made three
things better: borders are native styles instead of nested rects, panels are one element
instead of two, and the vestigial uGUI asmdef reference is gone. Two things to know:

- **The corrupting parchment (spec §5) got harder.** UI Toolkit has no per-element custom
  material, so when `_Corruption` becomes real it will be painted into the parchment texture
  (or the panel's render target), not driven through a material property. The palette and
  layout already obey the rules the effect inherits.
- **The IMGUI layer still wins.** The TransitionVeil and the F1/F2 debug overlays draw over
  the runtime panel (verified: SnapCovered blacks out the HUD completely), so transitions
  and respawns curtain the UI exactly as before, with no port needed there.
...
30. **A stale asset reference serializes as {fileID: 0}, as silently as null.** The first
    UI-Toolkit scene build assigned `UIDocument.panelSettings` from the PanelSettings instance
    held from BEFORE `NewScene` ran; the saved scene came out with `m_PanelSettings:
    {fileID: 0}` and the HUD rendered nothing — no error, no warning, an empty screen with a
    perfectly healthy element tree behind it (`resolvedStyle` all NaN is the tell: a tree
    with no panel never lays out). The builder now re-loads the asset by path AFTER the scene
    churn, assigns, and then greps the saved file for `{fileID: 0}` — a generated scene whose
    load-bearing reference can vanish silently earns a post-save proof, not trust.
